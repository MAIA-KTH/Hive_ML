...
def get_id_label(item):
    if 'non-pCR' in item:
        subject_id = item.split('non-pCR/')[1].split('_cropped')[0]
        label = 0
    elif 'pCR' in item:
        subject_id = item.split('pCR/')[1].split('_cropped')[0]
        label = 1
    elif 'NewBatch' in item:
        subject_id = item.split('NewBatch/')[1].split('_cropped')[0]
        label = 10000
